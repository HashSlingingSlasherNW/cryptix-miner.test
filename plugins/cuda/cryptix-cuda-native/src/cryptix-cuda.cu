...
__constant__ uint8_t final_cryptix[32] = { 0xE4, 0x7F, 0x3F, 0x73, 0xB4, 0xF2, 0xD2, 0x8C, 0x55, 0xD1, 0xE7, 0x6B, 0xE0, 0xAD, 0x70, 0x55, 0xCB, 0x3F, 0x8C, 0x8F, 0xF5, 0xA0, 0xE2, 0x60, 0x81, 0xC2, 0x5A, 0x84, 0x32, 0x81, 0xE4, 0x92};
...
// memory-hard mechanism
__device__ __inline__ void apply_memory_hard_technique(uint8_t* product, uint8_t* nibbles) {
    __shared__ uint8_t memory_table[MEMORY_TABLE_SIZE];  // shared memory
    uint32_t index = 0;

    // Initialize shared memory
    for (int i = threadIdx.x; i < MEMORY_TABLE_SIZE; i += blockDim.x) {
        memory_table[i] = 0;
    }
    __syncthreads();  
    ...
    for (int i = 0; i < 32; i++) {
        uint16_t sum = 0;
        for (int j = 0; j < 64; j++) {
            sum += nibbles[j] * matrix[2 * i][j];
        }

        // Non-linear memory accesses
        for (int _ = 0; _ < 12; _++) {
            index ^= (memory_table[(index * 7 + i) % MEMORY_TABLE_SIZE] * 19) ^ ((i * 53) % 13);
            index = (index * 73 + i * 41) % MEMORY_TABLE_SIZE;

            uint32_t shifted = (index + i * 13) % MEMORY_TABLE_SIZE;
            memory_table[shifted] ^= (sum & 0xFF);
        }
    }

    // Final XOR with memory table
    for (int i = 0; i < 32; i++) {
        int shift_val = (product[i] * 47 + i) % MEMORY_TABLE_SIZE;
...
extern "C" {
    ...
    __global__ void heavy_hash(const uint64_t nonce_mask, const uint64_t nonce_fixed, const uint64_t nonces_len, uint8_t random_type, void* states, uint64_t *final_nonce) {
        ...
        if (nonceId < nonces_len) {
            ...
            // Memory-Hard
            apply_memory_hard_technique(product, sha3_hash);

            // Final xor with final_cryptix
            for (int i = 0; i < 32; i++) {
            ...
            hash(heavyP, hash_.hash, product);
