...
__constant__ static const uint8_t final_x[32] = { 
...
__device__ void hash_nibbles(uint8_t *hash, uint8_t *nibbles) {
    for (int i = 0; i < 32; i++) {
        nibbles[2*i] = hash[i] >> 4;
        nibbles[2*i+1] = hash[i] & 0x0F;
    }
}

__device__ void xor_final_x(uint8_t *hash) {
    for (int i = 0; i < 32; i++) {
        hash[i] ^= final_x[i];
    }
}
...
extern "C" {
    ...
    __global__ void heavy_hash(const uint64_t nonce_mask, const uint64_t nonce_fixed, const uint64_t nonces_len, uint8_t random_type, void* states, uint64_t *final_nonce) {
        ...
        if (nonceId < nonces_len) {
            ...
            uint8_t nibbles[64] = {0};
            hash_nibbles(hash_.hash, nibbles);

            uint32_t product1, product2;
            #pragma unroll
            for (int rowId = 0; rowId < HALF_MATRIX_SIZE; rowId++) {
                amul4bit((uint32_t *)(matrix[(2*rowId)]), (uint32_t *)(nibbles), &product1);
                amul4bit((uint32_t *)(matrix[(2*rowId+1)]), (uint32_t *)(nibbles), &product2);
                product1 >>= 6;
                product1 &= 0xF0;
                product2 >>= 10;
