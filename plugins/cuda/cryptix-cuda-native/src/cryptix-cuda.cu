...
extern "C" {
    ...
    __global__ void heavy_hash(const uint64_t nonce_mask, const uint64_t nonce_fixed, const uint64_t nonces_len, uint8_t random_type, void* states, uint64_t *final_nonce) {
        ...
        if (nonceId < nonces_len) {
            ...
            // Determine nonce based on random type
            switch (random_type) {
                case RANDOM_LEAN:
            ...
            // Perform the first Keccak hash
            hash(powP, hash_.hash, input);

            // Perform the second Keccak hash without modifying the data
            uint256_t second_hash = hash_;
            hash(powP, second_hash.hash, second_hash.hash);

            // Use the result of the second hash for further processing
